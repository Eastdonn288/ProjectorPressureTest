...
- **「文件还是好的吗」用 `git` 验,不要用 `grep` 验(v2.12.0 补)**:**`git` 是白名单之外的另一个独立读进程,它能正确读出明文。** 所以在 `grep` 已经 0 命中的情况下,`git diff --stat <file>` / `git diff <file>` 照样打出**干净、可读的文本 diff** —— 这就是判定"文件内容完好、只是 grep 瞎了"的**最快铁证**,比抄一份 `.txt` 出来比对省事得多。2026-09-20 实测:`grep -c FastAPI server.py` = **0**,而同一刻 `git diff server.py` 打出完整的两行改动、`git show :server.py | grep -c FastAPI` = **3**。⚠ **别把"git 能看"读成"这文件没事所以随便 Edit"** —— `git` 只证明**磁盘内容**是对的,`Edit` 走的仍是它自己那个读路径,壳在它那边照样生效。
- **同一会话内状态会翻转(v2.12.0 实测,`perf_monitor.py`)**:那个文件在 2026-09-20 会话**开头是明文**(`Read`/`Grep`/`Edit` 全程可用,靠它改了上千行),而**同一次会话里自己用 Python 写完版本号之后,grep 就变成 0 命中了**。⇒ **"我这一轮已经 Read/Edit 过它了"不能作为"下一轮还能 Edit 它"的依据** —— 每次碰之前重新测,尤其**在你自己写过它之后**。
...
- **壳会中途翻转,而且`Read` 会谎报行数(v2.13.0 补,2026-09-20 实测)**:同一个 `scripts/perf_monitor.py`,**同一次会话内**先是明文(能被 Grep 正常搜到、被 Read 正常读出),中途翻成密文(`Grep` 0 命中、`Read` 返回乱码),**而文件本身完好无损**(`git diff` 干净、`py_compile` 通过、直接 `python` 跑得通)。⇒ 两个操作纪律:① **每一轮碰它之前重新测一次**,不许沿用上一轮的结论(**包括"我刚才才改过它就是明文"**);② **行数异常本身就是壳的指纹** —— 明文时该文件 4800+ 行,带壳时 `Read` 报一个几百行的数。**看到某个源文件行数少得离谱,先怀疑壳,别怀疑代码被删了。**
- **改了文件后,`Read` 报的行数/内容可能是壳造成的幻觉,而 `py_compile` 才是真的**:shell 化之后一切校验走 Python(`py_compile` / `--selftest` / 直接运行),不要用 `Read` 出来的一段文本当作"文件现在长这样"的证据。
...
## #49 — `cmd wifi status` 里 `IP:` 和 `SSID:` 各出现两次，第二次是占位符

- **症状**:解析器读到 `SSID: <unknown ssid>` / `IP: null`，把一次**正常联网**判成"未关联"或"已关联但没 IP" —— 报告里凭空多出一段不存在的中断。
- **根因**:`cmd wifi status` 的输出里有**两块**几乎同名的记录:`WifiInfo: SSID: "WCS-...", ..., IP: /172.16.0.189, RSSI: -64, ...`,以及后面的 `NetworkCapabilities: [ ... TransportInfo: <SSID: <unknown ssid>, ..., IP: /172.16.0.189, ..., RSSI: -65 ...> ...]`。**`IP:` 和 `SSID:` 这两个键各出现两次，而且两块的值会不一致** —— 本机实测 `WifiInfo` 报 `RSSI: -65 / Link speed: 702Mbps`,同一时刻的 `TransportInfo` 报 `RSSI: -66 / Link speed: 585Mbps`;更早一次实测 `WifiInfo` 是 `IP: null` 而 `TransportInfo` 已经是 `IP: /172.16.0.189`。**`TransportInfo` 里的 `SSID:` 还会是占位符 `<unknown ssid>`。**
- **规则**:**必须锚定行、只取第一条 `WifiInfo:` 行**。`perf_monitor.parse_wifi()` 的做法是找**第一个 strip 后以 `WifiInfo:` 开头**的行,再在这一行的 body 上做 `, ` 切分 + `: ` 键值切分(`setdefault`,先到先得)。**绝不能用 `re.search(r"IP: ([^,]+)")` 这类全文搜索** —— 它会命中哪一块取决于两块谁先出现,而顺序不是契约。
- **反例(仓内已有)**:`scripts/wifi_reboot_stress.py:108` 的 `"ssid:" in low` 是**大小写不敏感的全文包含**,它同时匹配 `SSID: <unknown ssid>` 这个占位符 —— 拿它当"已连接"的判据,会在设备没连上任何 AP 时判成已连接。
- **同类**:与 `gpu_clk` 的"必须按行取独立整数行,不能用 `re.search(r'(\d+)')`"(§12.1 的两处易错 ②)是同一类错 —— **输出格式里有多个同名 token 时,全文搜索是在赌顺序**。
- **位置**:`scripts/perf_monitor.py` 的 `parse_wifi` / `_wifi_info_fields`;`scripts/wifi_reboot_stress.py:108`
- **来源**:v2.12.0(真机实测两次,见 §12.10)

## #50 — 一个"只该用在 A 分支"的前缀写成了无条件，所有断言都还是绿的

- **症状**:HTML 报告里**每一段 WiFi 中断的时长都带 `≥` 前缀**,包括已经恢复的那些。`≥30.0` 读作"至少断了 30 秒" —— 但窗口闭合时这个 `≥` 是**没有依据的**:闭口窗口的跨度是 `t2 − t1`(断网前最后一次成功采样 → 恢复后第一次成功采样),真实时长落在 `(跨度−P, 跨度+P)` 区间内,**既不是下界也不是上界**。
- **根因**:渲染代码写的是期望里的条件表达式,实际是 `dur_cell = f'≥{...}'` —— **无条件**。而它**恰好长得完全正常**:格式正确、没有 `None`、没有崩、HTML 能开、`≥` 确实出现在该出现的那一页上,所以"开着的窗口要带 `≥`"那条断言**照样通过**。
- **为什么危险**:失败模式是**输出格式良好但语义是假的**。一次真实只有 10 秒、正好跨在一个采样点两侧的瞬断,会印成 `≥30.0` —— 一个**虚假的下界**,而它看起来比真空还确定。
- **规则**:① 断言必须**同时钉住两侧** —— 不只断言 `≥30.0` 在开着的窗口里出现,**还要断言 `≥60.0` 不在闭口的窗口里出现**(只测"该有的有"测不出"不该有的也有");② 断言**要落在具体的单元格上,不是整页**。第一版写成 `"≥" not in page`,结果被 `coverage` 那个**无关的**注释里的 `≥80%` 打红 —— 测试挂在错误的原因上,和测试挂着一样糟。锚点用 `≥60.0` / `>60.0<` 这种**值本身**当锚。
- **验证方式(必做)**:把 `≥` 改回无条件 → 新断言**必须变红** → 再改回来。**加完断言不做这一步,等于不知道它测的是不是真的。**
- **同类**:与 #42 同源 —— **形状断言看不见语义死掉的输出**。#42 是通道整条死了、断言全绿;这条是输出活着、但内容是假的、断言也全绿。
- **位置**:`scripts/perf_monitor.py` 的 `format_result_html` 六、小节 `dur_cell`
- **来源**:v2.12.0(C 组断网注入实跑的报告里发现)

## #51 — 同一个开关量被两个调用点各拿一半，漂移之后文档说的就不是真的了

- **症状**:WiFi 降级(连续 3 个到期 tick 读失败 → `wifi_enabled = False`)之后,**每一拍仍然把 `@@WIFI` 段发动到设备上** —— 白花几十毫秒,只是解析时被 `due_sec` 丢弃,所以**报告里看不出任何异常**。连带后果:文档里那句「此后不再发 `@@WIFI` 标记」**是假的**,`--probe-cost` 量到的"降级省下的那部分开销"也不存在。
- **根因**:`build_tick_command(nonce, tiers, guard, gpu_enabled)` 收的是 **`gpu_enabled`**,而同一次 tick 里的 `due_sec()` 闭包传给 `section_due(...)` 的是 **`wifi_enabled`**。**两个调用点、两个形参、各拿一半** —— 加 wifi 时只改了其中一个。更讽刺的是 `build_tick_command` 的 docstring 明写"把 `wifi_enabled` 做成关键字限定,就是为了让调用点漏传时大声报错",**但它有默认值 `= True`,所以一声没吭**。
- **为什么危险**:失败模式是**功能正确、性能退化、文档失真**三位一体,而且**没有任何断言看得见**。这类漂移不会崩、不会报错、报告全绿,只会在某一天有人拿 §12.1 的字面陈述去核对代码时才发现。它与 #42 是**同族**:#42 是"谓词写反 ⇒ 整条通道静默关闭",这条是"参数漏传 ⇒ 该关的东西静默没关"。
- **规则**:① **一个开关量只能有一个来源** —— 这里是 `Monitor.off_sections()` 算出的**一个 `frozenset`,`算一次,两个调用点共享**;`gpu_enabled` / `wifi_enabled` 两个形参被它**取代**(不是新增第三个)。② **形状上让它不可能漂移**,而不是"记得两处都改":两个函数现在收的是同一个对象。③ **正面断言**:`--selftest` 有一条对若干 `off_sections` 取值、断言 `build_tick_command` 与 `section_due` 对**每一段**的取舍**完全一致** —— 这条挡的正是本类。④ **关键字限定形参不要带默认值**:带了就等于把"漏传"变成"静默用默认值",docstring 的警告形同虚设。
- **验证方式(必做)**:把 `section_due` 的谓词里那个 `name not in off_sections` 条件**取反** → 新断言**必须变红**(实测 15 条同时红)→ 再改回来。这与 #42 的"反身验证"是同一套纪律。
- **位置**:`scripts/perf_monitor.py` 的 `build_tick_command` / `section_due` / `Monitor.off_sections` / `Monitor.tick`
- **来源**:v2.13.0(做监控项勾选时,为"哪几段不发"重构开关量时发现 —— 这个 bug 从 v2.12.0 就存在)

## #52 — "证据不足 ⇒ 放行"的分支分不清「没勾选」和「跑太短」

- **症状**:`perf_monitor` 有两道门在证据不足时**主动放行**:`fg_due < 2` 时门 8 判 `pass`(detail 写 "only 0 foreground read(s) - none expected");未设 `watch_pkg` 且前台数据为空时,门 9 以 `switches=0 pkgs=0` 判 `pass`。单看这两处**完全合理** —— 一次 3 拍的短跑确实不该因为"还没轮到前台"而变红。但**一旦用户取消勾选「前台应用」,这两条分支就会让报告声称一个从未被采集过的通道是健康的**。而这恰恰是新功能要达到的反面:用户取消勾选,是想让报告**承认**这次没看它。
- **根因**:那个分支的判据是「**数据太少**」,但输入里**没有任何东西区分「太少」和「根本不会有」**。两个完全不同的原因塌进了同一个布尔条件。同族的先例是 #44(「没出现过」和「出现过又消失」必须分开判)。
- **规则**:① **任何"证据不足 ⇒ pass"的分支,都必须先排除"证据根本不会被采集"** —— 这次的做法是在九道门都建好之后、`worst` 之前按 id 把受影响的门**覆盖成 `inconclusive`** 并写明原因,门 id 仍是 1..9(门表契约不动)。② **覆盖要排在 `worst` 之前** —— `INCONCLUSIVE` 优先级最高,所以取消勾选**不可能**让一次跑看起来更好。③ **不要用"新增一个伪门"来传达这件事**:伪门进 `caps` 不进 `gates`,而 `capped_by` 全仓无人渲染 —— 加了也没人看得见。范围事实由 `SCOPE` 行 + 被覆盖门的 detail 两处承载。
- **为什么危险**:失败模式是**假绿**,而且是最坏的一种 —— 报告不是漏报一个坏通道,是**给一个没人看过的通道盖章**。读者不会去怀疑一条写着"pass"的行。
- **验证方式(必做)**:这条断言在**未改动的代码上必须失败**(现状是静默 `pass`)。写完把覆盖逻辑注释掉 → 断言变红 → 改回来。
- **同类**:与 #44、#42 同族 —— 都是"**这件事没发生过**"与"**这件事发生了但结果是空**"被同一个条件放行。
- **位置**:`scripts/perf_monitor.py` 的 `judge`(门 8 的 `fg_due < 2` 分支、门 9 的 `switches=0 pkgs=0` 分支、`MON_GATE_OF` 覆盖)
- **来源**:v2.13.0(做监控项勾选时发现 —— 这条不是新引入的 bug,是既有逻辑在新输入下的暴露)

## #53 — `--probe-cost` 量到的不是设备的常数,是"这台设备此刻在忙别的"

- **症状**:同一台设备、同一天,`--probe-cost` 的 FULL 变体依次报 **318.3 / 314.4 / 428.6 ms**;两天前是 383.9,再往前是 644。单看两次相邻的测量,会得出"这次改动让每拍多了 N 毫秒"的结论 —— 而实际上大部分是设备自己的背景状态在动。**占空比因此从来不是一个常数**,而 §12.1.1 里那串数字是过去某一刻的**快照**。
- **根因**:这个工具量的是**复合 adb 调用的墙钟**,而墙钟取决于设备当时在干什么(它本来就是被测的那台机器,后台有桌面、有解码、有各种守护进程)。而变体是**逐个量完 reps 再换下一个**的 —— 于是每个变体都采到了**不同时刻**的背景状态,相邻变体之间的差值里**混进了背景漂移**。
- **实测(2026-09-20,设备 `EE88885D8DEF08004CD1`,4 核)**:空载 FULL **428.7 ms**;用 6 个 `timeout 120 yes` 把 4 核压到 ~98% 之后 FULL **877.5 ms**(整个 tick 翻倍,T=2.0 的占空比 9.48% → **19.01%**)。但要看清楚**是哪一部分变了**:NTC 自己的 delta **几乎不动**(空载 +73.0 ms → 负载 +66.4 ms),而 WiFi 的 delta 从 +68.6 爆炸到 **+213.3 ms**,且 min/median 散布极大(622 vs 1090 ms)。**"整体翻倍"不等于"每一项都翻倍"**。
- **规则**:① **只有相邻行的差值可比**;绝对毫秒数不可跨天比较,写进文档的占空比必须**连测量日期与当时背景一起写**(空载 / 负载)。② 要比较两个变体(比如"加 NTC 前后")时**交错取样**(round-robin:变体 A 一次、变体 B 一次、再回到 A),让背景漂移在**差值**里对消 —— 一个变体连量 5 次再换下一个,量到的是五个不同时刻。③ **要问"跑一次真实跑批要花多少"看负载下的那个总额**(FULL 428.7 → 877.5 ms)、空载只是基线;**但要问"某个通道值多少毫秒",负载下的逐通道 delta 根本不能用**(见下 —— 极差 220~255 ms,还出现过负号),那种问题空载下 n 次取区间都未必答得上来。**"负载下更重要"这句话只对总额成立,对 delta 不成立。** ④ **不要用它给"某次改动引入了开销"作证**:改动前后的两组数**必须交错量**,否则结论不可信。
- **② 现在由代码保证,不再是"用的人要记得"**(v2.13.1,`perf_monitor.py` 1.3.2):`run_probe_cost` 的内外两层循环已对调成**轮次在外、变体在内**,统计与打印挪到后面独立一遍。规则一旦只写在文档里就等于没写 —— 这条规矩在写成文字的第二天就被自己违反了。**调用次数 / sleep 次数 / runtime 全不变**,代价为零。
- **但它"有没有用"这件事,实测没量出来 —— 不许写成"重复性变好了"**:一开始拿 round-robin 的 3 次(极差 6.8)去比文档里块采样的 3 次(极差 34.9),看起来好了 5 倍 —— **那是错的比较**,两组来自不同时刻的背景。补做了一次**受控 A/B**(同一天两种取样**交替**各 3 次):round-robin 的 ntc delta 是 `+51.8 / +58.6 / +68.7`(极差 **16.9**),块采样副本是 `+63.3 / +64.5 / +71.8`(极差 **8.5**)—— **块采样那组反而更紧**。n=3 什么都说明不了,所以两边都不能下结论。可用的证据只有:`round-robin 的 7 次落在 +51.8 ~ +74.0`,`块采样的 6 次落在 +51.5 ~ +86.4`,散度同量级。
- **负载下又做了一遍,同样没量出优势**(6 个 `timeout 400 yes`,~66% idle,交替各 n=6):ntc delta —— round-robin `+36.9 ~ +284.5`(极差 **247.6**)、块采样 `+18.1 ~ +243.8`(极差 **225.7**);wifi delta —— round-robin `−7.2 ~ +216.1`、块采样 `−23.0 ~ +232.3`。**两种顺序各出现了一次负的 wifi delta** —— 一个纯成本差值出现负号,说明行间散布已经**大于通道本身的代价**。**空载时这个数只是不可靠,负载时它直接是错的。**
- **负载下真正的主导误差不是行间漂移,是单次调用自己的抖动**(这也是"交错为什么没用"的解释):同一次测量里,`FULL` 这一行自己的 5 次取样是 **min 627.0 / max 924.6 ms(组内极差 297.6)**,`FULL+WIFI` 298.2、`FULL+WIFI+NTC` 198.0。**组内极差(200~300)和组间极差(220~255)同量级** —— 每一行自己就没测准,那么"两行是否同时刻"自然不再是瓶颈。**改取样顺序治不了这个,只有加 reps;加多少没做实验**(TODO §6.20)。
- **那为什么还留着**:它去掉的是一个**系统性**误差项,不是噪声 —— 块采样把"两行之间那段时间设备在忙别的"**整段记进差值**,而差值正是这张表唯一的产品。机制上交错只会让估计更接近无偏,而代价是零。**"这次没量出来" ≠ "这项误差不存在"**;**但反过来也一样 —— 两次受控实验(空载 3+3、负载 6+6)都没量出改善,而且两次都是块采样那组极差更小,"实测证明它更稳"这句话是假的,不许写。**
- **同类**:与 #40(判定的分母不能用"实测耗时",要用"拍序号")是同一个毛病 —— **把一个会被环境影响、因此会漂的量,当成了设备常数**。
- **位置**:`scripts/perf_monitor.py` 的 `run_probe_cost` / `--probe-cost`;[PERF_MONITOR_V2.md](PERF_MONITOR_V2.md) §12.1.1 的占空比表;README 的占空比段
- **来源**:v2.13.1(用户追问"这个占空比不是常数又是什么问题")

## #54 — 归档伴随文件的规则是「**stem 前缀 + 后缀白名单**」，不是"搬走报告目录里的所有东西"

- **症状**:新产物 `perf_<设备>_<时间>.samples.temps.png` 出现在 `reports/stress-test/perf/` 里，**归档目录里没有它**，`archive/` 看起来一切正常 —— 用户打开归档找图，找不到。而同一批产物里的 `.temps.csv` **在**归档里。同一次运行、同一个目录、同一个前缀，一个进了一个没进。
- **根因**:`server.py` 的 `_companion_files` **不是目录 glob**。它要求三条**同时**成立：同目录 + 文件名以 `<报告 stem>.` 开头 + **后缀在白名单元组里**。白名单当时是 `(".csv", ".html")`。`.temps.csv` 两条都合格，`.png` **前缀合格、只有后缀被挡**。`scripts/_pptp_report.py` 把这种结局叫 **"a silent loss"** —— 它和"文件根本没生成"在归档里长得一模一样。
- **为什么危险**:失败模式是**安静丢失**，而且丢的是**用户明确要过的那件东西**（"图表输出路径保持在 archive 的同目录"）。没有报错、没有日志、报告全绿、归档不空 —— 除了那个文件不在，一切都对。这类问题只会在有人专门去找那个文件时暴露。
- **规则**:① **判断一个新产物能不能进归档，看的是"它的名字以谁开头"和"后缀在不在白名单里"，不是"它在不在报告目录"**。名字不合格 → 改文件名；后缀不在白名单 → 放宽白名单。**这是两件事，只做一件就是安静丢失。** ② **放宽白名单是平台公共逻辑**（影响所有脚本的归档），必须在 CHANGELOG 与 [DECISIONS.md](DECISIONS.md)（D-67）里写明，不能只改代码。③ **元组旁那段注释是活文档**：它记的是"为什么这个元组是活的、加一种新产物要动这里" —— 不要因为"这条路径下现在没有别的 png"就删掉它。
- **这次为什么敢放宽**:实测确认**当前全仓没有任何脚本往报告目录写 `.png`**（唯一的 PNG 生产者是浏览器，它直接写进归档目录、绕过 `_companion_files`），所以放宽不会多收任何既有文件。**但这条结论只对今天的仓库成立** —— 将来若有脚本写同名前缀的 `.png`，它会被一并收进归档，而那本来就是正确行为。
- **验证方式（必做）**:真机跑一次，断言**两件事都成立**：①归档目录里 `*.temps.csv` 与 `*.temps.png` **都在**；②`reports/stress-test/perf/` 里**这两个文件都没残留**。只看 ① 会漏掉"复制而非移动"的 bug，只看 ② 会漏掉本次这条。**只看归档"看起来完整"是最没用的检查** —— 本条的整个症状就是归档看起来完整。
- **同类**:与 #45（产物命名要带 stem）同族 —— 那条管"名字能不能认出来"，这条管"名字合不合归档的规则"。
- **位置**:`server.py` 的 `_companion_files`；`scripts/_pptp_report.py` 的 `html_path_for`（"silent loss" 这个说法出自那里）
- **来源**:v2.14.0（做 NTC 自动换算时发现 —— 图已经生成了，只是进不了归档）

## #55 — 一次"实测可用"如果量的是**会话状态**而不是**设备属性**，它就会在别人手里失效

- **症状**:NTC 两个 sysfs 节点在 v2.13.0–v2.14.0 上**恒为 off**、没有读数、控制台**没有任何原因**。用户在**同一台**设备上手工 `cat` 那个文件**能出数**，脚本里读不到。文档白纸黑字写着"免 root，world-readable"。
- **根因**:两句都"真"过一次。节点的权限位确实是 `-rw-r--r-- root root`（所以"world-readable"是对权限位的正确描述）；当初那次实测"裸 `cat` 可用"也确实成功过 —— **因为那时设备的 `adbd` 是 root 在跑**，`adb shell` 直接落进 root 域，于是量出了"不需要 `su`"。**这个结论量的是"当时的 adbd 身份"，不是"这个节点能不能读"。** 设备一重启 `adbd` 回到 `shell` 域，SELinux（**Enforcing**，节点标签 `u:object_r:sysfs:s0`）就把 `shell` 域读 `sysfs` 拒了，而且**权限位一点没变** —— 所以照着权限位去核对，会得出"节点没问题，那一定是别的地方坏了"。
- **为什么危险**:两个独立原因叠在一起，把失败做成了**完全静默**：
  1. 被 SELinux 拒绝的 `cat` 把 `Permission denied` 写进 **stderr**、**stdout 零字节**；
  2. 探针只取 stdout，于是打印一行空的 `(empty)` 和一个裸 FAIL。
  **日志里没有任何东西指出是权限问题**，读者的第一反应是"这设备没有这个节点"，而不是"这条命令不够权限"。一个静默的失败会自己长出一个错误的结论。
- **规则**:① **权限位和访问控制是两回事。** 判一个节点"能不能读"，**必须在真正会被用的那个身份下读一次**（`adb shell id` 看域，别只看 `ls -l`）。② **凡是写进文档的"不需要提权"，都要写明"在什么身份下测的"** —— 否则它下次过期时没有人知道。③ **探针的职责包括说出失败原因。** 命令失败时 stdout 常常是空的，**原因在 stderr**；只回显 stdout 的探针等于把诊断能力丢掉。回显 stderr 时**只在 stdout 为空时才看**（正常通道保持一行摘要，不变吵），并且**强制转 ASCII**（D-07，设备本地化的报错会在 GBK 控制台上炸掉打印）。
- **不是"文档写错了"就能了事**:这条的代价不是少一个指标，是**读者会以为设备不支持**。所以修法必须同时让**失败能自己解释自己** —— 一个会说原因的探针，比一条写在 README 里的前提条件可靠得多。
- **同类**:与 #13（GPU 节点不在标准路径，换设备先 `--probe`）、#51（同一个开关量被两个调用点各拿一半）同族 —— 都是"文档/断言描述的东西，和真正决定行为的东西不是同一个"。#53 记的是"量到的不是设备的常数，是设备此刻的状态"，**本条是它在权限维度上的同一形状**。
- **位置**:`scripts/perf_monitor.py` 的 `NTC_CMD` / `_probe_raw()`；说明见 [PERF_MONITOR_V2.md](PERF_MONITOR_V2.md) §12.11.6；裁决 D-68
- **来源**:v2.14.1（用户报"我能 cat 出来，但脚本显示 NTC off、没任何打印"）


## #56 — 用来说明身份的装饰(底色描边)在标记密到一定程度时会变成**橡皮擦**

**现场**:`tools/ntc_convert.py` 的温度曲线,14.5 小时 / 1737 个读数。图上**曲线没了**,只剩几个孤立的点
和糊在一起的一团标记。CSV 一个字节没错。

**为什么**:**剂量**。每个点带一圈 2 pt(4.17 px)的底色描边(`markeredgecolor=surface`)—— 这是 dataviz
规范明确要求的("重叠标记加 2px 底色环",让点压在线上也看得清),**但它只在标记分得开的时候成立**。
那个跑里 1737 个点摊在 1468 px 上 = **一个点 0.81 px**,相邻标记完全重叠;描边于是从"分隔"变成"覆盖":
它把底下的**台阶线**和左右邻居的点面整片涂掉。轴内彩色像素:只有线 **11930**,加上点只剩 **1974**。

**规矩**:

1. **一条"关于分辨率"的画法规则,必须先算清楚它要占多少像素。**"每个读数打一个点"在 2 分钟的跑上是对的,
   在 14 小时的跑上是**自毁**。任何按元素逐个画的装饰,都要问一句"最坏情况下一个元素摊到几个像素"。
2. **用来说明身份的装饰,方向可能是反的。**描边、间隙、留白的用途是**分开**相邻标记;当它们分不开时,
   同一个装饰就只剩下"覆盖"这一个效果。**装饰成不成立的前提是间距,不是装饰本身。**
3. **先量剂量,再逐个禁用 artist。**这次第一反应是"哪个 artist 把线盖掉了",于是逐个禁用
   (legend / grid / annotate / set_title / ax.text / subplots_adjust / margins / set_axisbelow /
   tick_params)—— **全是绿的**,因为每个 artist 单独画都对,合起来才坏。改从"最密元素的间距 vs 像素"
   (0.81 px vs 16.7 px 的点径)入手,一步到位。
4. **数据对 + artist 对 ≠ 图画对。**坏在**光栅化**那一步时,所有"断言对象 / 断言数据 / 断言形状"的测试
   都会全绿。要钉住它只能**读回产物**(这次是数 PNG 里的系列色像素),并且**故意把 bug 复现一遍**
   (强行打开点 → 线塌陷),否则"永远不画点"会照样通过所有断言。

## #57 — 轮询间隔**就是**测量分辨率;测不到的窗口不会留下一行"0 秒",它会留下一行"什么都没发生"

**现场**:`power_cycle_stress.py` 每 `poll_interval_sec`(默认 2 s)跑一次 `adb devices`,通过"设备消失/回来"
推断断电时长。用户真实的继电器节奏是 **30 s OFF**,但继电器节奏**后续要变**。

**为什么**:探测器的时间分辨率**等于**采样间隔,这件事没有绕过去的办法 —— 加高精度时间戳也没用,因为
`adb devices` 这个调用本身就带秒级抖动。后果有两条,而且**形状不同**:

1. **短于一个间隔的断电完全看不见** —— 不是"测成 0 秒",是**那一轮根本没有发生过**:设备从没离开过设备表,
   状态机就停在 `WAIT_OFF`,报告里连一行都不会有。
2. **测得到的时长,两端各差最多一个间隔** —— 真实 30 s 的窗口可能被记成 28 或 32。

**规矩**:

1. **分辨率必须写进报告,而且要用"能测到什么 / 不能测到什么"的措辞**,不能只写一个 ± 数字。
   本项目对 WiFi 那条链路已经定了同一口径(见 #49 邻域):**看到一条中断 = 真的断过;没看到 = 不能反推没断过**。
2. **不要把分辨率不足谎报成精度** —— 报告里写 `±1 个嗅探间隔`,不要写 `± interval/2`。我们**不知道**设备
   是在间隔的哪一刻离开的。
3. **估不出数的时候写"不可测",不写 0**。起跑时设备可能**已经**是断电状态(继电器正在循环中),这一轮
   的断电时长**没有起点** ⇒ 报 `n/a`。写 0 会被读成"没断电",那是反的。
4. **一个仪表的分辨率是它的固有属性,不是待修的缺陷** —— 记录它、在报告里标注它,别为了"更准"去加设备端
   埋点(那会改变被测对象)。

## #58 — 在压测里,**设备离开 adb 是正常状态,不是错误**

**现场**:`power_cycle_stress.py` 的每一轮都以"设备从 `adb devices` 里消失"开始。

**为什么容易搞错**:平台别处(WS 断开、设备卡变灰、串口重连)全都把"设备没了"当成**异常**处理,所以很自然
会想给这里也加一条离线提示 —— 横幅、toast、日志告警、或者把那一轮标成异常。**但这里设备消失是"被测对象
本身"**:断电就是要它消失。给它报警等于每 10 分钟报一次自己制造的假故障。

**规矩**:

1. **先问"这个状态是症状还是设定"**。同一个状态("设备不在 adb 里")在负载测试里是**设定**,在平台运维里
   才是症状 —— 判据是**谁**造成的它。
2. **别重复造已有的 UI**。前端**已经**有「临时离线」合成卡(以及电池脚本的「放电关机」卡)—— 设备不在
   `adb devices` 但仍有在跑的任务时,卡片不会消失,而是变成虚线琥珀边。**直接用它,不要为这个脚本再加一套**。
3. **"读不到"和"确实没有设备"必须分开判**。`parse_adb_devices()` 拿不到 `List of devices attached` 这一行时
   返回 `None`(读失败),设备表为空时返回 `{}`(真的没设备)。**合并这两者**会让一次 adb 抖动伪造出
   一整个"断电→上电"循环,报告里每一轮都是假的。
4. **"发现状态变化"要连续确认**,不要单次采样就下结论。同理,OFF 要**连续两次**读不到才相信(`OFF_CONFIRM_POLLS = 2`)
   —— 否则脚本会对着一个**从没离开过**的设备发开机按键。
5. 与 D-09 / D-36 的关系:**那不是"不许提醒",是"不许把正常状态报成异常"**。运行**前**的前置条件
   (如本脚本的"上电方式必须为 Direct")是另一回事,它走 D-70 的 `hint`。

## #59 — ini 里 `delay_ms` 是"按下**之后**的等待";循环时无害,单次截断时它是个坑

**现场**:新脚本 `power_cycle_stress.py` 要在开机后**单次**跑一遍 IR 序列。手边能抄的序列是
`ir_sequences/idle_keepalive.ini`,而它的最后一步 `delay_ms = 1200000`(**20 分钟**)—— 那是 keepalive 的
"隔 20 分钟再按一次"。

**为什么**:`ir_runner.run_step()` 的收尾是
...
即 **`delay_ms` 是"按完这一下之后等多久"**,不是"按之前等多久"。而且**它挂在"这一份副本还没按完"上** ——
所以一份 `count=1` 的副本**一次都不睡**。这意味着两件事:

1. **单次截断跑时,`delay_ms` 变成序列内部的等待**,20 分钟会真的插在开机后动作里,整轮卡死在那儿,
   而 stdout 上没有任何一行说"我在等"。**单次跑不许直接复用 keepalive 序列。**
2. **"谁负责节奏"必须写清楚**。`perf_monitor` 那边的做法是**构造 `count=1` 的副本、自己按 `delay_ms` 逐次
   `sleep`** —— 本脚本照抄了这个形状(见 D-43 的实现要点)。**一旦某处改成 `count=N` 原样丢给 `run_step`,
   `delay_ms` 的语义就从"两次按键之间的间隔"变成"最后一下之后还睡一觉",行为完全不同**,而它**不会报错**。

**规矩**:① 用别人的 ini 之前先看最后一步的 `delay_ms`;② 任何"单次跑一遍"的需求,**不要**复用为循环设计的
序列文件;③ 报告里记下"本轮的 IR 是不是单次",别让读者从时长上猜。
