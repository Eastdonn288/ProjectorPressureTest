...
**四条已批准(DO NOT REVERT)**:① **线程内 `import ir_runner`**,不起子进程;② 先给一个 `KEYCODE_MEDIA_PLAY` 模板,**之后要能随便选 ini、随便改**(`key_ini` select 扫 `ir_sequences/*.ini`,跳过 `_` 开头的临时候选);③ **30 分钟**一次(由 ini 的 `delay_ms` 决定);④ ~~**只有 perf_monitor 需要** —— 其余 7 个脚本与平台逻辑**零改动**~~ **已被 D-71 修订**(2026-09-23):`power_cycle_stress.py` 是第二个线程内 `import ir_runner` 的脚本,「只有 perf_monitor」这句不再成立。①②③ 三条**原样不动**;平台侧仍然零改动。
...
**④ 的修订(2026-09-23,见 D-71)**:原文说「只有 perf_monitor 需要」,那是当时的事实 —— 2026-09-22 之前
确实只有一个脚本要按键。`power_cycle_stress.py` 加了第二个(开机后动作),这句话就成了错的,**所以改的是
这句话,不是那四条规矩**。①②③ 一条没动,而且**新脚本落地的形状比 perf_monitor 更窄**:

- **线程 vs 主线程**:perf_monitor 用 daemon 线程,因为它是**周期性的**(每 30 分钟一次),不能占住主循环。
  power_cycle_stress 的 IR 是**每轮一次**的一次性动作,跑完就结束,所以**直接在主线程同步执行** —— 而且
  主线程反而**更可中断**:`ir_runner` 内部的 `subprocess.run` 没有 `timeout=`,CTRL_BREAK 能立刻打断主线程,
  而**投给工作线程的控制台事件永远不来**(这正是本条禁止 `run_loop` 的同一个理由)。daemon 线程的理由
  (「硬停后子进程会活下来继续按键」)针对的是**子进程**,主线程同步执行根本不创建子进程,所以那条理由不适用。
- **还是 `count=1` 副本逐次调 `run_step`**(本条实现要点的原话),节奏仍归调用方;但**主线程上用的是
  `time.sleep`** —— 它在主线程是可被 CTRL_BREAK 打断的,不需要 `stop.wait` 那套(那是给工作线程用的)。
- **失效只进 `status`**:`ir_status` / `ir_sent` / `ir_failed` 进报告 `config`,**永远不改变判定** —— 与本条
  「不加 gate」完全一致。
- **`run_loop` 仍然禁止**,`ir_runner.py` 本身仍然**零改动**(D-29)。
...
> **⚠ 2026-09-22 修订:① 的「真实读数打点」已被 D-69 加上条件 —— 点密到分不开时不画点。**
> **D-63 原文保留(不抹决策史);本修订只动 ① 的打点,阶梯 / 逐通道断线 / 配色 / 副标题密度 / 单轴全部不变。**
...
### D-68 NTC 两个 sysfs 改走 `su 0`;探针失败时**必须说出原因**(2026-09-21)
> **本条推翻 D-57 ~ D-63 那一段里"NTC 免 root / 不需要 `su`"的表述。** 那句话在写下的那天是真的,但它量的是**当时 `adbd` 是 root**,不是"这个节点对 `shell` 域可读"。原文保留,这里加推翻标记 —— 不抹决策史。

**症状**:v2.13.0–v2.14.0 上 NTC **恒为 off**,没有读数,**控制台上没有任何原因**;而用户在同一台设备上手工 `cat` 那个文件**能出数**。

**用户裁决(原话)**:「直接写死 `su 0`」。

**根因(两条,缺一不可)**:
1. **裸 `cat` 读不到。** 两个节点权限位确实是 `-rw-r--r-- root root`,但本板 SELinux **Enforcing**、节点标签 `u:object_r:sysfs:s0`,`adb shell` 落在 `u:r:shell:s0` —— **该域被拒绝**;`su 0`(`u:r:su:s0`)可以。**旧结论的来路**:当初实测"裸 `cat` 可用"时,那台设备的 `adbd` 正好是 root,`adb shell` 直接落 root 域。**它量的是会话状态,不是设备属性** —— 设备一重启 `adbd` 回到 `shell`,NTC 就静默失效了,而**权限位一个字节都没变**。
2. **失败被丢掉了。** `_probe_raw()` 只取 stdout;被拒绝时 `cat` 把 `Permission denied` 写进 **stderr**、stdout **零字节** —— 于是探针打印一行空的 `(empty)` 和一个裸 FAIL,**没有任何东西指出是权限问题**。

**落点**:
- `NTC_CMD` 与 GPU 那条路对齐,写死 `su 0`:`timeout -k 1 2 su 0 cat <LCD> <LED>`。`su` 在这条命令上不打印任何东西(实测),帧标记不受影响。
- **`NTC_GUARD_S` 不跟着设备 guard 走**(GPU 那条跟):这个 guard 只在驱动卡死时才会咬到,而 `su 0` 的 +60.6 ms 远不触及它。
- `_probe_raw()` 在 **stdout 为空**时才把 stderr 的**第一行**打出来,一行、纯 ASCII(D-07)。**只在为空时看** —— 正常的通道仍只有一行摘要,不会变吵。stderr 也空时打 `no output and no error - rc=N`。

**这次接受的取舍(明写)**:`su 0` 单次读中位数 **139.5 → 200.1 ms(+60.6 ms/拍)**(SLOW 档一拍一次 ⇒ T=2.0 约 **+0.10 pp** 占空比);且**设备不能 root 就丢 NTC**(降级关闭,与 GPU 同一处理)。旧行为是"名义免 root、实际读不到",新行为是"要 root、但真的读得到"。

**判据(比修法更重要)**:① **权限位和访问控制是两回事** —— 判一个节点能不能读,要在**真正会被用的那个身份**下读一次(`adb shell id` 看域,别只看 `ls -l`)。② **文档里每一句"不需要提权"都要写明"在什么身份下测的"**,否则它过期时无人知道。③ **探针的职责包括说出失败原因** —— 命令失败时原因在 **stderr**,只回显 stdout 的探针等于把诊断能力丢掉。这条的代价不是少一个指标,是**读者会以为设备不支持**。

**禁止**:把 `su 0` 退回裸 `cat`(除非有设备实测证明 `shell` 域可读);把 `_probe_raw` 改回只回显 stdout;在 stderr 回显里放进未净化编码的文本(GBK 控制台会炸,D-07);因为"+60.6 ms 很贵"就把 NTC 移出 SLOW 档(它在 SLOW 档,一拍只读一次)。

详见 [PITFALLS.md](PITFALLS.md) #55 与 [PERF_MONITOR_V2.md](PERF_MONITOR_V2.md) §12.11.6。

### D-69 点密到分不开时**不画采样点** —— 修订 D-63「真实读数打点」那句(2026-09-22)
> **本条修订 D-63 的「① `阶梯 + 真实读数打点`」里的打点部分。** 阶梯、逐通道断线、配色、副标题密度、
> 单轴 —— 全部不变。D-63 原文保留,已在其上加推翻标记 —— 不抹决策史。

**触发**:用户 2026-09-22 报「csv 的数据是 ok 的,但是生成出来的图片曲线完全丢失了,节点都糊在一起」。
在归档 `archive/perf/20260921-180514_perf_monitor_435E54A1BB0D08004C10/` 上复现。

**事实(实测,不是推断)**:那一次 **14.5 小时 / 1737 个读数**摊在 1468 px 的绘图区上 = **一个点 0.81 px**;
每个点带 `markeredgewidth=2` pt(**4.17 px**)的 `markeredgecolor=surface` 描边。**0.81 px 间距下相邻标记
完全重叠**,那圈描边从"分隔"变成"橡皮擦",把底下的台阶线连同邻居的点面一起涂掉。按系列色统计绘图区内像素:
只有线 **11930**、只有点 **1845**、**线 + 点(改动前)1974**、线 + 点去掉描边 **39704**。
数据侧完全正常(26047 行、`t_sec` 单调无重复、零空单元格、`ntc_st` = `h:24310 / f:1737`)—— **坏在剂量上**。

**用户裁决**:三个选项(密集时整个不打点 / 点随密度自适应缩小 / 按屏幕像素抽稀)里选了**「不打点了」**。

**判据(不许改写成别的)**:一个点的**外沿**直径 = `MARKER_PT + MARKER_EDGE_PT` = 10 pt = **20.83 px**
(描边骑在标记边界上,向内吃 `mew/2`、向外占 `mew/2`,所以外沿直径正好是两者之和;而**可见点面**是
`MARKER_PT - MARKER_EDGE_PT` = 6 pt)。该通道真实读数的**中位**间隔 × px/s ≥ 该阈值 → 画;否则**整个不画**。
`px_per_second()` 从**数据跨度 + 版面常数**算,**不许**读 `ax.get_xlim()` —— 画点发生在任何 artist 之前,
那时坐标轴还没有数据范围,读到的是默认的 `(0,1)`。

**副标题必须说明**:不打点的通道进 `_describe_run()` 的 `dotless`,副标题追写「采样点密过屏幕像素,未画点」。
**禁止**让点的消失成为一个需要读者自己推断的静默变更(短跑有点、长跑没有,不说就是骗)。

**逐通道判定**:判据挂在通道上,不是一个全局开关 —— 一个节点读数稀、另一个密时,稀的那个照旧有点。

**如实记下的代价**:30 秒档下约 **40 分钟以上**的跑就看不到采样点了,图上只有台阶线。读数、状态、温度在
`*.temps.csv` 里一个不少,副标题仍然报中位采样周期;丢的只是"哪一拍是真实读数"这个视觉提示,而它在
0.81 px 的密度下本来也不可读。**短跑一个点没少。**

**禁止**:把点画小一点而不是不画(亚像素点不是读者学到的那个标记);按屏幕像素**抽稀**(点还在,但个数
不再等于读数个数,读者会按点数推断采样周期);给点换一种编码(线型 / 纹理)来"保留"密度;把阈值从
`MARKER_PT + MARKER_EDGE_PT` 里拆出来写成独立常数 —— 那会让它和真正画出的点脱钩,而自检有一条**量点径**的
断言(量 `MARKER_PT - MARKER_EDGE_PT`)专门钉这件事。

### D-70 脚本可自述一条 `hint` 前置条件,常驻参数弹窗顶部(2026-09-23)

**触发**:`power_cycle_stress.py` 有一条**跑之前就必须满足**的前提 —— 设备上电方式必须是 `Direct(上电自动
开机)`。不满足的话脚本不会报错,它只会**一直等**一个永远不来的开机,整夜白跑。用户裁决:「需要在脚本页面
显示"需将设备上电方式改为 Direct"作为提醒」。

**为什么这不是 D-09 / D-36 / D-47 的违反**(这是本条存在的全部理由,后来者删它之前先读这三行):

| 被否掉的旧方案 | 本方案为什么不是它 |
|---|---|
| D-09 / D-36 禁的是**离线横幅 / toast / 状态条** | 那些是**运行时状态**提醒,而"离线"在压测里是**正常状态**(见 D-71)。`hint` 是**运行前的前置条件** —— 静态、不随时间变、只在用户**主动打开**那个参数弹窗时出现一次,不占据任何常驻版面 |
| D-47 禁前端为某个脚本**硬编码参数表单** | `hint` 是**脚本自述的数据**。前端渲染逻辑对任何脚本一视同仁,`static/app.js` 与 `static/index.html` 里**不出现任何脚本名、不出现那句中文** |

**形状(三处,边界都在这)**:

- **脚本侧**:`--dump-params` 打印 `{"fields": [...], "hint": "<中文>"}`。`json.dumps` 默认
  `ensure_ascii=True`,中文被转义 ⇒ **这一行仍是纯 ASCII**(D-07 的硬边界不破)。源码里 hint 与 label 一样
  是字面中文,与 8 个兄弟脚本逐字一致(D-07 约束的是 **stdout 与注释**,不是 UI 文案)。
- **服务端**(唯一一处平台改动,`server.py` 的 `/api/scripts/{name}/params`):把 `hint` 从脚本输出里取出
  (缺失或非 `str` ⇒ `""`)并放进响应 `result = {"script":…, "fields":…, "hint":…}`。**所有脚本统一多一个
  `hint: ""`**,所以前端可以无条件读 `hint`。`fields` 的语义一个字节没变 ⇒ `has_params` 嗅探、`saveParams`、
  报告参数表**全部不受影响**。
- **前端**(`static/app.js` ×2 处 + `index.html` ×1 处):`loadParamsSchema` **缓存整个响应**(以前只留
  `r.fields`,那正是 hint 会静默消失的地方);`openParamsModal` 里在**长度早退检查之前**填 hint(否则一个
  "只有 hint 没有字段"的脚本会连 hint 都看不到),用 **`textContent` 而非 `innerHTML`**。DOM 上是一个
  `<div class="seq-warn" id="params-hint" hidden>`,**放在 `#params-form` 之外** —— 表单是 `max-height`
  可滚动容器,放进去了会被滚走。样式**复用** `.seq-warn`,`style.css` 零改动。

**判据**:`hint` 是**脚本自述**的,平台不许知道任何一个脚本名;`hint` **不是参数** —— 它不进 `fields`、
不进参数表单、不进报告的参数表(`_pptp_report.render_params_table` 遍历的是 `params_schema`,天然免疫),
也不进 `state.deviceParams` 的 localStorage。

**已知边界(明写,不修)**:参数弹窗**只由脚本卡片的那个入口打开**,而那个入口只在脚本声明了
`--dump-params` 时才可点 —— 所以**一个没有参数的脚本即使有 hint 也显示不出来**。本条的用途就是"带参数的
脚本附带说明",不是"给任意脚本挂公告牌"。

**禁止**:把 `hint` 塞进 `fields` 当一条只读字段(会污染参数表与 localStorage);把文案硬编码进前端
(D-47);用 `innerHTML` 渲染它;把它做成会随运行状态变化的提示(那就变成 D-09/D-36 禁掉的那类东西了);
在服务端给它加键白名单或校验(D-70 的契约就是"原样转发的字符串",缺省 `""`)。

### D-71 硬开关机压测:继电器是**自主**的,脚本只嗅探、绝不控制(2026-09-23)

**背景**:`power_cycle_stress.py` 是本仓**第一个观测外部电源**的脚本,也是第一个**不由自己发起重启**的压测
脚本 —— 兄弟 7 个都是自己 `adb reboot`,自己知道时间起点。这里的电源由一只**外部继电器**按自己的定时器
通断,用户的周期是 10 min ON / 30 s OFF,**并且明确说了后续节奏不固定**。要验的是"上电自动开机"这条
链路里"偶尔开不了机"的稀发故障 —— 这种故障靠人工开关机撞不出来,必须整夜跑。

**核心裁决(本条的心脏)**:**循环节奏是观测量,不是断言**。脚本把观测到的断电时长 / 开机耗时**记录并报告**,
**永不**拿它跟任何配置值比对。所以 `PARAMS` 里**没有** `on_sec` / `off_sec` 这类字段 —— 加了就等于把
"节奏不固定"这句话作废。报告里必须写明测得的时长是**观测**、并标注**测量分辨率**。

**脚本对电源的权限 = 零**。它不发任何控制命令,`relay` / 电源控制在本仓**不存在**(见 ARCHITECTURE §9)。

**状态机与两个超时的分工**(两个超时**都不**拿继电器的 10min/30s 做任何比较):

| 阶段 | 判据 | 归谁管 |
|---|---|---|
| `WAIT_OFF` 设备在 adb 里 → 等它消失 | 无超时(只打心跳) | 断电由继电器决定,等多久都正常 |
| `WAIT_BACK` 设备已消失 → 等它回到 adb | `off_watchdog_sec`(默认 1800 s) | **看门狗,不是节奏断言**。只在"继电器被拔了 / USB 掉了 / 设备彻底砖了"时兜底;触发 ⇒ 该轮 FAIL **并结束整轮跑** |
| `WAIT_BOOT` 已在 adb → 等 `sys.boot_completed == "1"` | `boot_timeout_sec`(默认 120 s) | **这才是本脚本的判定**。设备出现在 adb ⇒ 电已经通了,这一段确实归我们判。触发 ⇒ 该轮 FAIL,**继续下一轮**(失败累积成比率) |
| `POST_BOOT` 等 `post_boot_wait_sec` → 读 WiFi → 跑一遍 ini | 无超时 | 用户裁决:等完 `post_boot_wait_sec` **再**读 WiFi(不是 boot_completed 那一刻);该参数的**默认值**随后由用户从 70 s 改成 **25 s**,裁决本身不变 |

**"设备离开 adb 是正常状态,不是错误"**:这是本条最容易被后人改坏的地方。设备每轮消失是**被测对象本身**,
所以**不许**加任何离线告警(D-09 / D-36)。而且前端**已经有**「临时离线」合成卡(D-25 同一手法),
**不重复实现**。同理有两条实现细节被钉死:

- **OFF 要确认两次**(`OFF_CONFIRM_POLLS = 2`,连续两次读不到才相信)。单次读失败就把"没读到"当成"断电了",
  会让脚本对着一个**从没离开过**的设备发开机按键。
- **"读不到" 与 "确实没有设备" 必须分开**:`parse_adb_devices` 拿不到 `List of devices attached` 这行时
  返回 **`None`**(读失败),设备表为空时返回 **`{}`**(真的没设备)。把两者合并会让一次 adb 抖动伪造出一整个
  断电→上电循环,报告里的每一轮就都是假的。

**WiFi 只记录,永不判定**(用户裁决):`wifi` 那一列进报告、进逐轮明细表,但**不参与 PASS/FAIL**。
判定的**充要条件**是"存在开机超时或看门狗触发"。这条在报告里明写,免得后来者把它当成漏判;`--selftest`
里有一条断言**直接钉它**(构造"WiFi 全未连接但每轮都开成了"⇒ 必须 `OK`)。

**IR 序列是单次截断,不是循环**(用户 2026-09-23 追加要求,与 perf_monitor 相反):开机后的动作是
**每轮跑一遍**的,不是 keepalive。实现上 `import ir_runner` **在线程内、在主线程同步执行**,`run_step`
每次收一份 **`count=1` 的副本**(所以 `run_step` 末尾那句 `if n < step.count: sleep` 不触发,节奏归调用方),
**`run_loop` 永不参与** —— 它的唯一出口是本线程的 `KeyboardInterrupt`,而 Windows 永远不会送来。IR 失败
**只进 `ir_status` / `ir_sent` / `ir_failed`,永远不改变判定**(D-43:不加 gate)。`ir_runner.py` 零改动(D-29)。

**判据**:① 观测量与断言必须分开 —— 一个"节奏不固定"的系统里,把观测值当断言 = 脚本在断言它自己都不知道
的东西。② **供电是否恢复,只有 adb 认账**(所以 `WAIT_BOOT` 的起点是"回到 adb",不是"时间到了")。
③ 分辨率是**固有**的,不是缺陷:轮询间隔**就是**探测器的分辨率,短于一个间隔的断电**完全看不见**,测得的
时长两端各差最多一个间隔 —— 报告与 PITFALLS 写明,**不试图修**。

**禁止**:给脚本加任何电源/继电器控制接口;把 10min/30s 写成参数或写进判据;把 WiFi 状态接进 PASS/FAIL;
让 `run_loop` 参与本轮 IR;把"读失败"和"没有设备"合并;在设备离线时加任何 UI 提醒。

详见 [PITFALLS.md](PITFALLS.md) #57(分辨率)/ #58(离线是正常状态)/ #59(单次截断的 `delay_ms`)。
